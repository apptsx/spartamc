...
public class SkyWinsLeaderboard extends LeaderboardHologram {

    public SkyWinsLeaderboard(Player player, Location location) {
        ...
        for (int i = start; i < start + itemsPerPage; i++) {
            if (i < memberList.size()) {
                ...
                lines.add(String.format("§e%s. %s §7- %s",
                        ...
                        Util.formatNumber(member.getTotalWins())
                ));
            } else {
