...
function criarModal(tipo) {
    ...
    if (tipo === 'suporte') {
        ...
        const tipoPunicaoInput = new TextInputBuilder()
            .setCustomId('tipo_punicao')
            .setLabel('Tipo da punição')
            .setPlaceholder('Mute ou Ban')
            .setStyle(TextInputStyle.Short)
            .setRequired(true)
            .setMaxLength(10);
        ...
        const tempoInput = new TextInputBuilder()
            .setCustomId('tempo_punicao')
            .setLabel('Tempo da punição')
            .setPlaceholder('Ex: 7 dias, permanente')
            .setStyle(TextInputStyle.Short)
            .setRequired(true);
        ...
        modal.addComponents(
            new ActionRowBuilder().addComponents(tipoPunicaoInput),
            new ActionRowBuilder().addComponents(idInput),
            new ActionRowBuilder().addComponents(motivoInput),
...
async function handleModal(interaction, client) {
    ...
    try {
        ...
        const channel = await interaction.guild.channels.create({
            name: `${isRevisaoPunicao ? '🔍' : emojiCanal}・${isRevisaoPunicao ? 'revisao' : nick}`,
        ...
        const campoIds = ['necessidade', 'produto', 'problema', 'duvida', 'id_punicao', 'motivo_punicao',
                         ...
                         'tipo_punicao', 'tempo_punicao', 'provas'];
        ...
        const ticketEmbed = new EmbedBuilder()
            ...
            .setTitle(isRevisaoPunicao ? '🔍 Revisão de Punição' : `📋 Ticket #${channel.id.slice(-4)} • ${tipo.toUpperCase()}`)
        ...
        // Botões
        const buttons = isRevisaoPunicao
        ...
        await channel.send({ 
            content: isRevisaoPunicao ? `${interaction.user} | ${staffRole}` : `${interaction.user} | ${staffRole}`,
            embeds: [ticketEmbed],
            components: [buttons]
        });
        ...
        const confirmEmbed = new EmbedBuilder()
            ...
            .setDescription(
                isRevisaoPunicao
                    ? `Sua solicitação de revisão de punição foi enviada para análise.\n\nA equipe analisará e você receberá uma resposta no privado.`
