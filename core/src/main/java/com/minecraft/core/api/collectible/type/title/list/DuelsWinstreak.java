...
public class DuelsWinstreak extends TitleCollectible {

    public DuelsWinstreak() {
        super("Winstreak Duels", TitleCategory.DUELS, CollectibleRarity.COMUM, Arrays.asList(com.minecraft.core.account.context.objects.rank.type.RankType.MEMBER), System.currentTimeMillis());

        setIcon(Item.of(Material.NAME_TAG));
        setLore(Arrays.asList(
                "§7Mostra sua winstreak atual nos Duels."
        ));
    }

    @Override
    public String getTitle() {
        return "§eWinstreak: §b0 §8(Duels)";
    }
    ...
    public String getDynamicTitle(Player player) {
        ...
        for (ArcadeCategory arcade : ArcadeCategory.of(ServerType.DUELS)) {
            total += member.getStats(arcade).getWinStreak();
        }
        return "§eWinstreak: §b" + total + " §8(Duels)";
    }
}
